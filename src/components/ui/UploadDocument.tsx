        ...
        reader.onloadend = async () => {
            try {
                ...
                if (onOcrComplete && ocrSchema) {
                    ...
                    setApiVerificationStatus('verifying');
                    const extractedData = await api.extractDataFromImage(selectedFile.type, ocrSchema, docType);
    ...
    return (
        <div className="w-full">
            ...
            <div className="flex items-center gap-2 mb-2">
                ...
                {verificationStatus === true && <span title="Verified"><CheckCircle className="h-4 w-4 text-sky-400" /></span>}
                {/* FIX: Replaced the `title` prop on `lucide-react` icons with a wrapping `<span>` to provide a tooltip, as the `title` prop is not supported directly. */}
            ...
            <div
                className={`
                    ...
                    ${file ? 'p-2 border border-solid border-gray-600' : 'p-6 border-2 border-dashed border-sky-400'}
                ...
                {file ? (
                    ...
                    <>
                        ...
                        <div className="flex items-center gap-4 mt-4">
                            <label htmlFor={inputId} className="fo-btn-primary rounded-full !bg-sky-500/20 !text-sky-300 hover:!bg-sky-500/30 flex items-center gap-2 cursor-pointer !px-4 !py-2">
