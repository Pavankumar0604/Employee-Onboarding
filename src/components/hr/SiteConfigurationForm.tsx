...
import type { SiteConfiguration, Organization, Entity } from '../../types/mindmesh';
...
const siteConfigSchema = yup.object({
  ...
  agreementDetails: yup.object({
    ...
    softCopy: yup.mixed<UploadedFile>().nullable().optional(),
    scannedCopy: yup.mixed<UploadedFile>().nullable().optional(),
  ...
  siteOperations: yup.object({
    ...
    form6Document: yup.mixed<UploadedFile>().nullable().when('form6Applicable', { is: true, then: schema => schema.required('Document is required') }),
    ...
    minWageRevisionDocument: yup.mixed<UploadedFile>().nullable().when('minWageRevisionApplicable', { is: true, then: schema => schema.required('Document is required') }),
    holidays: yup.object({
    ...
    tools: yup.object({
      dcCopy1: yup.mixed<UploadedFile>().nullable().optional(),
      dcCopy2: yup.mixed<UploadedFile>().nullable().optional(),
      list: yup.array().of(yup.object({ id: yup.string(), name: yup.string().required(), brand: yup.string().optional(), size: yup.string().optional(), quantity: yup.number().nullable().min(0), issueDate: yup.string().required(), picture: yup.mixed<UploadedFile>().nullable().optional() })),
    }).optional(),
    sims: yup.object({
...
const SiteConfigurationForm: React.FC<SiteConfigurationFormProps> = ({ isOpen, onClose, onSave, organization, allClients, initialData }) => {
    ...
    const { register, handleSubmit, control, watch, setValue, formState: { errors }, reset } = useForm<SiteConfiguration>({
        resolver: yupResolver(siteConfigSchema) as Resolver<SiteConfiguration>,
        defaultValues: {
          siteOperations: {
            ...
            <div className="bg-card rounded-xl shadow-card w-full max-w-4xl my-8 animate-fade-in-scale" onClick={e => e.stopPropagation()}>
                <form onSubmit={handleSubmit(onSubmit)} className="p-6">
                    ...
                    <div className="max-h-[60vh] overflow-y-auto pr-2 space-y-4">
                        ...
                        {activeTab === 'Agreement' && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                                <Controller name="agreementDetails.fromDate" control={control} render={({ field }) => (<DatePicker label="From Date" id="agreementDetails.fromDate" value={field.value} onChange={field.onChange} error={(errors.agreementDetails as any)?.fromDate?.message} />)} />
                                <Controller name="agreementDetails.toDate" control={control} render={({ field }) => (<DatePicker label="To Date" id="agreementDetails.toDate" value={field.value} onChange={field.onChange} error={(errors.agreementDetails as any)?.toDate?.message} />)} />
                                <Input label="Renewal Interval (Days)" id="agreementDetails.renewalIntervalDays" type="number" registration={register('agreementDetails.renewalIntervalDays')} error={(errors.agreementDetails as any)?.renewalIntervalDays?.message} />
                        ...
                        {activeTab === 'Site Operations' && (
                           <div className="space-y-4">
                                    ...
                                    <div>
                                      ...
                                      {form6Applicable && (
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2 pl-6 border-l-2 border-accent-light">
                                          <Controller name="siteOperations.form6ValidityFrom" control={control} render={({ field }) => (<DatePicker label="Validity From" id="form6ValidityFrom" value={field.value} onChange={field.onChange} error={(errors.siteOperations as any)?.form6ValidityFrom?.message} />)} />
                                          <Controller name="siteOperations.form6ValidityTo" control={control} render={({ field }) => (<DatePicker label="Validity To" id="form6ValidityTo" value={field.value} onChange={field.onChange} error={(errors.siteOperations as any)?.form6ValidityTo?.message} />)} />
                                          <div className="md:col-span-2">
                                ...
                                <AccordionItem title="Holidays">
                                   <div className="space-y-4">
                                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                          <Input label="Number of Days" id="holidaysNumberOfDays" type="number" registration={register('siteOperations.holidays.numberOfDays')} error={(errors.siteOperations?.holidays as any)?.numberOfDays?.message} />
                                          <Select label="NH Salary Payment" id="holidaysSalaryPayment" registration={register('siteOperations.holidays.salaryPayment')} error={(errors.siteOperations?.holidays as any)?.salaryPayment?.message}><option value="">Select</option><option>Full Payment</option><option>Duty Payment</option><option>Nil Payment</option></Select>
                                          <Select label="NH Billing" id="holidaysBilling" registration={register('siteOperations.holidays.billing')} error={(errors.siteOperations?.holidays as any)?.billing?.message}><option value="">Select</option><option>Full Payment</option><option>Duty Payment</option><option>Nil Payment</option></Select>
                                      </div>
                                      <div>
                                ...
                                <AccordionItem title="Tools & Costing">
                                    <div className="space-y-4">
                                        <Input label="Link to Costing Database" id="costingSheetLink" registration={register('siteOperations.costingSheetLink')} error={(errors.siteOperations as any)?.costingSheetLink?.message} />
                                ...
                                <AccordionItem title="SIMs & Equipment">
                                     <div className="space-y-4">
                                      ...
                                      <Input label="Number of Sims issued to the site" id="simsIssuedCount" type="number" registration={register('siteOperations.sims.issuedCount')} error={(errors.siteOperations?.sims as any)?.issuedCount?.message} />
                                    </div>
                                </AccordionItem>
