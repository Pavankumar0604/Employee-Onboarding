...
import type { Organization, Asset } from '../../types/onboarding';
import { getAssets, getOrganizations, updateAsset } from '../../services/api';
...
const AssetDetailView: React.FC<{
    ...
    onBack: () => void;
}> = ({ site, initialAssets, onSave, onBack }) => {
        ...
        <form onSubmit={handleSubmit(handleSaveSubmit)}>
            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-6">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <Button type="submit" loading={isSaving}><Save className="mr-2 h-4 w-4" /> Save Changes</Button>
                </div>
            </div>
...
const AssetConfig: React.FC = () => {
    ...
    useEffect(() => {
        setIsLoading(true);
        Promise.all([getOrganizations(), getAssets()])
            .then(([sitesData, assetsData]) => {
                if (sitesData) {
                  setAllSites(sitesData.sort((a: Organization, b: Organization) => a.shortName.localeCompare(b.shortName)));
                }
                const groupedAssets: Record<string, Asset[]> = {};
                assetsData?.forEach(asset => {
                  if ((asset as any).siteId) {
                    if (!groupedAssets[(asset as any).siteId]) {
                      groupedAssets[(asset as any).siteId] = [];
                    }
                    groupedAssets[(asset as any).siteId].push(asset as any);
                  }
                });
                setAllAssets(groupedAssets);
            })
    ...
    const handleSave = async (siteId: string, assets: Asset[]) => {
        try {
            await Promise.all(assets.map(asset => updateAsset(asset.id, asset)));
