...
const UserManagement: React.FC = () => {
    ...
    const fetchUsers = useCallback(async () => {
        setIsLoading(true);
        try {
            const data = await api.getAllUsersWithRoles();
    ...
    const handleConfirmDelete = async () => {
        if (currentUser) {
            try {
                await api.deleteUserProfile(currentUser.id);
    ...
    const getRoleName = (role: string | undefined | null) => {
        if (typeof role !== 'string') {
            return 'Unknown Role'; // Or an empty string, depending on desired display
        }
