...
const AttendanceRulesPage: React.FC = () => {
    ...
    const handleAddHoliday = (e: React.FormEvent) => {
        ...
        if (newHolidayName && newHolidayDate) {
            if (holidays.some((existingHoliday: Holiday) => existingHoliday.date === newHolidayDate)) {
                             ...
                             {holidays.length > 0 ? (
                                holidays.map((currentHoliday: Holiday) => (
                                    <div key={currentHoliday.id} className="flex justify-between items-center p-3 rounded-lg">
                                        ...
                                        <Button variant="icon" size="sm" onClick={() => removeHoliday(currentHoliday.id)} aria-label={`Remove ${currentHoliday.name}`}>
