...
import type { EnrollmentRules, DocumentRules, VerificationRules, SiteStaffDesignation } from '../../types/mindmesh';
...
import { Save, IndianRupee, Users, Edit, UserCheck, FileText } from 'lucide-react';
import { api } from '../../services/api';

const defaultDesignationRules: { documents: DocumentRules; verifications: VerificationRules } = {
    documents: {
...
const EnrollmentRules: React.FC = () => {
    ...
    useEffect(() => {
        api.getSiteStaffDesignations().then((data: SiteStaffDesignation[]) => {
            const uniqueDesignations = [...new Set(data.map((d: SiteStaffDesignation) => d.designation))];
    ...
    useEffect(() => {
        // When a new designation is selected, check if rules exist in the form state.
        // If not, initialize them to default values (all true for UI checkboxes)
        const currentRulesForDesignation = getValues(`rulesByDesignation.${selectedDesignation}`);
        if (!currentRulesForDesignation) {
            setValue(`rulesByDesignation.${selectedDesignation}`, {
                documents: Object.fromEntries(Object.keys(defaultDesignationRules.documents).map(key => [key, true])) as unknown as DocumentRules,
                verifications: Object.fromEntries(Object.keys(defaultDesignationRules.verifications).map(key => [key, true])) as unknown as VerificationRules,
            }, { shouldDirty: true });
        }
    }, [selectedDesignation, setValue, getValues]);
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 max-w-4xl mx-auto">
                        ...
                        <div className="space-y-3">
                            {Object.keys(defaultDesignationRules.documents).map(key => (
                                <Controller
                                    ...
                                    control={control}
                                    // defaultValue will be set by the useEffect above if rules don't exist
                                    render={({ field }) => <Checkbox id={`${selectedDesignation}-${key}`} label={key.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase())} checked={field.value ?? false} onChange={field.onChange} />}
                                />
                            ))}
                        </div>
                    </section>
            ...
            <div className="bg-card p-6 sm:p-8 rounded-xl shadow-card">
                ...
                <div className={`transition-opacity ${isEsiRuleEnabled ? 'opacity-100' : 'opacity-50 pointer-events-none'}`}><div className="max-w-sm"><Input label="ESI Eligibility Gross Salary Threshold" id="esiCtcThreshold" type="number" {...register('esiCtcThreshold', { valueAsNumber: true })} disabled={!isEsiRuleEnabled} /><p className="text-xs text-muted mt-2">Employees at or below this amount are eligible for ESI. Those above are eligible for GMC.</p></div></div>
            </div>
            ...
            <div className="bg-card p-6 sm:p-8 rounded-xl shadow-card">
                ...
                <div className={`transition-opacity ${isGmcRuleEnabled ? 'opacity-100' : 'opacity-50 pointer-events-none'}`}>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                        <Input label="GMC Salary Threshold" id="salaryThreshold" type="number" {...register('salaryThreshold', { valueAsNumber: true })} disabled={!isGmcRuleEnabled} />
            ...
            <div className="bg-card p-6 sm:p-8 rounded-xl shadow-card">
                ...
                <div className="space-y-4">
                    ...
                    <div className={`max-w-sm transition-opacity ${isEnforceLimitEnabled ? 'opacity-100' : 'opacity-50 pointer-events-none'}`}>
                        <Select label="Action When Limit is Reached" id="manpowerLimitRule" {...register('manpowerLimitRule')} disabled={!isEnforceLimitEnabled}>
