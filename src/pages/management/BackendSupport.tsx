...
const PriorityIndicator: React.FC<{ priority: PriorityKey }> = ({ priority }) => {
    const styles: Record<PriorityKey, string> = {
        Low: 'bg-blue-500',
...
type StatusKey = 'Open' | 'In Progress' | 'Pending Requester' | 'Resolved' | 'Closed';
...
const SupportDashboard: React.FC = () => {
        ...
        const fetchData = async () => {
            ...
            try {
                const [ticketsData, usersData] = await Promise.all([
                    api.getSupportTickets(), // Fetch actual support tickets
                    api.getUsers() // Assuming getUsers returns all users, which we filter for 'nearby'
                ]);
                setTickets(ticketsData as SupportTicket[]); // Now ticketsData is SupportTicket[]
                setNearbyUsers(usersData as User[]);
            } catch (error) {
    ...
    const openWhatsAppChat = (phone?: string) => {
        ...
        } else if (numberToCall.length === 11 && numberToCall.startsWith('0')) {
            // 11-digit number starting with 0, replace 0 with 91
            numberToCall = '91' + numberToCall.substring(1);
        } else if (numberToCall.length === 12 && numberToCall.startsWith('91')) {
            // Already in correct format
        } else {
            // Fallback for numbers that might already have a country code but are not 12 digits (e.g., from other countries)
            // Or if it's a malformed number. The safest is to assume it's a 10-digit number if it's long.
            const last10 = numberToCall.slice(-10);
            if (last10.length === 10) {
